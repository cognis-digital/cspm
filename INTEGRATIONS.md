...
**cspm** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cspm` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cspm ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cspm ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
cspm ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
cspm ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cspm` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cspm") for rec in records]   # records = your JSON output
