...
[![PyPI](https://img.shields.io/pypi/v/cognis-cspm.svg)](https://pypi.org/project/cognis-cspm/)
[![CI](https://github.com/cognis-digital/cspm/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/cspm/actions)
...
CSPM exists for one job — cloud security posture from a config export (public buckets, open sgs, weak iam) — and does it without a SaaS bill or heavyweight setup.
...
```bash
cspm --version
cspm scan .                      # scan the current project
cspm scan . --format json
cspm scan . --fail-on high       # non-zero exit for CI gates
cspm mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **ScoutSuite/Prowler**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`cspm` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
