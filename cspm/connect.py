"""Native cognis-connect emit for cspm — forward findings to any platform.

Maps cspm's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cspm ... --format json | cspm-emit --to stix
    cspm-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('check_id', '') + ': ' + rec.get('resource', '')
        out['severity'] = rec.get('severity', 'INFO')
        out['type'] = rec.get('check_id', '')
